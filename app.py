            ...
            if len(polys) > 0:
                ...
                if overlap_pairs:
                    ...
                    for a, b in overlap_pairs:
                        ...
                        for g in groups:
                            if a in g or b in g:
                                g.add(a); g.add(b); merged = True; break
                        if not merged:
                            groups.append({a, b})
                    changed = True
                    while changed:
                    ...
                    for grp in groups:
                        ...
                        errors.append({"type": "Overlap", "severity": "High",
                            "desc": f"Overlap group: features {flist}"})
                                ...
                                if gap.area > 1e-12:
                                    ...
                                    if gap_count <= 50:
                                        errors.append({"type": "Gap", "severity": "Medium",
                                            "desc": f"Gap #{gap_count} (area: {area_sqm:.1f} sqm)"})
                    ...
                    try:
                        if gdf.geometry.iloc[i].equals(gdf.geometry.iloc[j]):
                            ...
                            errors.append({"type": "Duplicate", "severity": "High",
                                "desc": f"Duplicate: feature {gdf.index[i]} and {gdf.index[j]}"})
                    except: pass
                ...
                try:
                    ...
                    if not geom.is_valid:
                        ...
                        errors.append({"type": "Self-Intersect", "severity": "High",
                            "desc": f"Invalid geometry (self-intersect) at feature {idx}"})
                except: pass
            print(f"   Self-intersections: {si_count}")

        # ============ 7. SPIKES (Angle detection) ============
                            ...
                            if angle < angle_threshold:
                                ...
                                if spike_count <= 50:
                                    errors.append({"type": "Spike", "severity": "Medium",
                                        "desc": f"Spike at feature {idx} (angle: {angle:.1f})"})
                except: pass
            if spike_count > 50:
                errors.append({"type": "Spike", "severity": "Medium",
